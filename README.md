...
IR Skill 不是只负责保存资料的工具。数据层负责让每次研究都能复用，Skill Reference 则按问题选择分析路径，并把数据、披露和研究材料转化为具体的研究判断。
...
对应的详细规则分别位于 [`references/long-term-trading.md`](references/long-term-trading.md)、[`references/medium-term-catalyst.md`](references/medium-term-catalyst.md)、[`references/short-term-trading.md`](references/short-term-trading.md) 和 [`references/deep-review.md`](references/deep-review.md)。结构化数据进入论证，但不替代公司、交易所、巨潮或监管机构的原始披露。
...
4. **规范化常用数据**：日线、估值、财务指标、股票基础信息、交易日历、指数、行业成分与权重等，会额外写入可查询的 SQLite 表。
...
`indicators` 的 `technical_snapshot` 会把末值与近期变化整理成趋势、动量、风险/位置和成交参与四组证据，便于短期分析稳定引用；它不产生综合分数或自动买卖信号。结构化数据不会被伪装成研究结论。它们是可带时点和口径继续分析的基础材料；公司财务、治理与重大事项仍应回到公司、交易所、巨潮或监管机构的原始披露核验。
...
日期目录和文件名前缀均使用报告的 `as_of`，格式为 `YYYY-MM-DD`。报告不与 SQLite 缓存、任务原件或可复用资料混放。Research Hub 从这个目录直接读取正式交付物，资料库则保存可长期复用的来源和事实材料。
...
## 可编辑的 Skill Reference：把系统换成你的投资风格

数据层、归档规则和 Wiki 校验可以长期复用，但你的投资方法不应被写死。[`SKILL.md`](SKILL.md) 是纯 Markdown 的 Skill Reference，可以直接修改为自己的研究框架。

| 需要定制的内容 | 建议修改的位置 |
| --- | --- |
| 核心因子、风险/流动性约束、证据门槛 | `SKILL.md` 的“核心护栏” |
| 长期、周期、催化、事件或技术信号的优先级 | `SKILL.md` 的“Reference 路由” |
| 行动标签、目标价格、撤销条件与复盘节奏 | `SKILL.md` 的“决策纪律” |
...
| 任务 | 读取的主模块 |
| --- | --- |
| 多年持有、商业质量、治理、资本配置、长期估值 | [`references/long-term-trading.md`](references/long-term-trading.md) |
| 约 3-6 个月的盈利、订单、供需、政策、产品或估值催化 | [`references/medium-term-catalyst.md`](references/medium-term-catalyst.md) |
| 一个月内交易、事件/宏观、技术面、动量、因子筛选、入场节奏或价格异动 | [`references/short-term-trading.md`](references/short-term-trading.md) |
| 公司、交易所、监管、政府或行业机构的网页、PDF、原始披露与政策资料 | [`references/external-evidence-sources.md`](references/external-evidence-sources.md) |
...
| 独立交叉质询或深度研究 | [`references/deep-review.md`](references/deep-review.md) |
